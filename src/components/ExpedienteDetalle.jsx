...
import { X, Users, Calculator, Bed, DollarSign, FileUp, TrendingUp, Save, Upload, Trash2, Plus, FileText, Pencil, Printer, FileDown, Paperclip, Eye } from 'lucide-react'
...
const ExpedienteDetalle = ({ expediente, onClose, onUpdate, onRefresh, clientes = [], initialTab, user = null }) => {
  ...
  useEffect(() => {
    ...
    const mapped = initialTab === 'pagosProveedores' ? 'cierre' : initialTab
    if (['grupo', 'cotizacion', 'pasajeros', 'cobros', 'facturacion', 'documentacion', 'cierre'].includes(mapped)) {
      setTab(mapped)
    }
  }, [initialTab])
