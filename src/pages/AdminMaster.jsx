...
import { Building2, Plus, Minus, RefreshCw, X } from 'lucide-react'
...
const AdminMaster = () => {
  ...
  const crearAgencia = async (e) => {
    ...
    let lim = Math.max(1, parseInt(String(limiteNueva), 10) || 1)
    setGuardandoNueva(true)
    const { error: err } = await supabase.rpc('master_crear_empresa', {
      p_nombre: nombre,
      p_limite_usuarios_staff: lim,
    })
    setGuardandoNueva(false)
    if (err) {
      alert(err.message || 'No se pudo crear la agencia.')
      return
    }
    setModalNueva(false)
    await cargar()
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-600 mt-1 text-sm">
            Gestión central de agencias. Los cambios de límite y estado se aplican al instante en la base de datos.
          </p>
        </div>
      ...
      {error && (
        <div className="rounded-xl border border-amber-200 bg-amber-50 text-amber-900 px-4 py-3 text-sm mb-6">{error}</div>
      )}
            ...
            <form onSubmit={crearAgencia} className="p-5 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 focus:ring-2 focus:ring-violet-500 outline-none"
                />
                <p className="text-xs text-slate-500 mt-1">Por defecto 1. Siempre mínimo 1.</p>
              </div>
              <div className="flex gap-2 pt-2">
                ...
                >
                  {guardandoNueva ? 'Guardando…' : 'Crear'}
                </button>
              </div>
            </form>
          </div>
