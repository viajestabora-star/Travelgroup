...
import { Users, Calculator, Calendar, Briefcase, AlertTriangle, Clock, CheckCircle, Globe, X, StickyNote, ChevronRight, ShieldCheck, Archive, UserCog } from 'lucide-react'
...
const Dashboard = ({ user = null }) => {
  ...
  useEffect(() => {
    if (esGestoria) {
      setCargandoCupoLicencias(false)
      setCupoLicencias(null)
      return
    }
    let cancelled = false
    const cargarCupo = async () => {
      setCargandoCupoLicencias(true)
      const resultado = await obtenerResumenLicenciasEmpresa(supabase, null)
      if (cancelled) return
      if (resultado.ok && resultado.resumen) {
        setCupoLicencias({ tipo: 'ok', ...resultado.resumen })
      } else {
        setCupoLicencias({ tipo: 'err', mensaje: resultado.error || 'No se pudo cargar el cupo de licencias.' })
      }
      setCargandoCupoLicencias(false)
    }
    cargarCupo()
      ...
      {!esGestoria && (
        <div className="mb-8 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-4">
            <UserCog className="text-sky-600" size={22} />
            <h2 className="text-lg font-bold text-navy-900">Licencias de tu agencia</h2>
          </div>
          {cargandoCupoLicencias ? (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {[1, 2, 3].map((i) => (
                <div key={i} className="h-16 bg-slate-100 rounded-lg animate-pulse" />
              ))}
            </div>
          ) : cupoLicencias?.tipo === 'err' ? (
            <p className="text-sm text-amber-800 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
              {cupoLicencias.mensaje}
            </p>
          ) : cupoLicencias?.tipo === 'ok' ? (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="rounded-lg border border-slate-100 bg-slate-50/80 p-4">
                <p className="text-xs uppercase tracking-wide text-slate-500 mb-1">Contratadas</p>
                <p className="text-2xl font-bold text-slate-900 tabular-nums">{cupoLicencias.contratadas ?? '—'}</p>
              </div>
              <div className="rounded-lg border border-slate-100 bg-slate-50/80 p-4">
                <p className="text-xs uppercase tracking-wide text-slate-500 mb-1">Usadas</p>
                <p className="text-2xl font-bold text-slate-900 tabular-nums">{cupoLicencias.usados ?? '—'}</p>
              </div>
              <div className="rounded-lg border border-slate-100 bg-emerald-50/80 p-4">
                <p className="text-xs uppercase tracking-wide text-slate-500 mb-1">Disponibles</p>
                <p className="text-2xl font-bold text-emerald-800 tabular-nums">{cupoLicencias.disponibles ?? '—'}</p>
              </div>
            </div>
          ) : null}
        </div>
